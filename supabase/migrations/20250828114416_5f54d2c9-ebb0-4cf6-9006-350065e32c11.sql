...
ADD COLUMN max_machines integer;

-- 为程序表添加备注字段（用于描述机器数量套餐）
ALTER TABLE public.programs
ADD COLUMN machine_limit_note text;

-- 为卡密表添加机器数量相关字段
ALTER TABLE public.card_keys
ADD COLUMN max_machines integer DEFAULT 1;

-- 为卡密表添加已绑定的机器码字段
ALTER TABLE public.card_keys
ADD COLUMN bound_machine_codes text[] DEFAULT '{}';

-- 为卡密表添加当前使用的机器数量字段
ALTER TABLE public.card_keys
ADD COLUMN used_machines integer DEFAULT 0;
